...
class ClientController extends Controller implements HasMiddleware
{
    ...
    public function formulario(Request $request)
    {
        ...
        Client::create([
            ...
            'phone' => $request->phone,
            'notes' => $request->notes,
            'user_id' => auth()->id() ?? 1, // Default user 1 if not logged in
        ]);

        // Enviar datos a n8n
        Http::post(env('N8N_WEBHOOK_URL', 'https://ia.telcored.cl/webhook/formularioarbitrajevar'), $request->all());

        return redirect('/#contact')->with('success_contact', '¡Mensaje enviado con éxito! Gracias, nos contactaremos a la brevedad.');
    }
}
